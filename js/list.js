    ...
    function renderPage() {
        ...
        if (curPage === allPage) {
            for (let i = 0; i < productData.length % showProNum; i++) {
                let j = (curPage - 1) * showProNum + i;
                str =
                    `
                    <li class="list-group-item" detail-id="${productData[j].goods_id}">
                        <div>
                            <p class="text-center">${productData[j].cat_three_id}</p>
                            <p class="picture"><img src="${productData[j].goods_small_logo}" /></p>
                            <p class="intro">${productData[j].goods_name}</p>
                            <p class="text-center">${productData[j].goods_price}
            ...
            for (let i = 0; i < showProNum; i++) {
                let j = (curPage - 1) * showProNum + i;
                // console.log(j, curPage, showProNum, i);
                ...
                str =
                    ...
                    <li class="list-group-item" detail-id="${productData[j].goods_id}">
                        <div>
                            ...
                            <p class="intro">${productData[j].goods_name}</p>
                            <p class="text-center">${productData[j].goods_price}
                            <a class="btn btn-default addToCart" role="button" data-id="${productData[j].goods_id}">加进购物车</a></p>
                        </div>
                    </li>
                `
                uls[i].innerHTML = str; //给list-group添加item
            ...
            if (curPage > 6) {
                ...
                if (curPage + 4 > allPage) {
                    for (let i = allPage - 10; i < allPage; i++) {
                        str += `<li><a href="#" data-bg="${i}>${i+1}</a></li>`
                    }
                } else {
